...
class html2text
{
    ...
    /**
     *  List of preg* regular expression patterns to Pesquisar for,
    ...
    /**
     *  List of pattern replacements corresponding to patterns Pesquisared.
     *
     *  @var array $replace
     *  @access public
     *  @see $Pesquisar
     ...
     *
     *  @var array $ent_Pesquisar
     *  @access public
     *  @see $ent_replace
     */
    public $ent_Pesquisar = array(
     ...
     *
     *  @var array $callback_Pesquisar
     *  @access public
     */
    public $callback_Pesquisar = array(
        '/<(a) [^>]*href=("|\')([^"\']+)\2([^>]*)>(.*?)<\/a>/i', // <a href="">
    ...
    /**
     *  List of preg* regular expression patterns to Pesquisar for in PRE body,
     *  used in conjunction with $pre_replace.
     *
     *  @var array $pre_Pesquisar
     *  @access public
     *  @see $pre_replace
     */
    public $pre_Pesquisar = array(
        "/\n/",
        "/\t/",
        '/ /',
        '/<pre[^>]*>/',
    ...
    /**
     *  List of pattern replacements corresponding to patterns Pesquisared for PRE body.
     *
     ...
     *
     *  First performs custom tag replacement specified by $Pesquisar and
    ...
    private function _converter(&$text)
    {
        ...
        // Run our defined tags Pesquisar-and-replace
        $text = preg_replace($this->Pesquisar, $this->replace, $text);

        // Run our defined tags Pesquisar-and-replace with callback
        $text = preg_replace_callback($this->callback_Pesquisar, array($this, '_preg_callback'), $text);

        // Strip any other HTML tags
        $text = strip_tags($text, $this->allowed_tags);

        // Run our defined entities/characters Pesquisar-and-replace
        $text = preg_replace($this->ent_Pesquisar, $this->ent_replace, $text);

        // Replace known html entities
        $text = html_entity_decode($text, ENT_QUOTES);

        // Remove unknown/unhandled entities (this cannot be done in Pesquisar-and-replace block)
    ...
    private function _build_link_list( $link, $display, $link_override = null)
    {
        ...
        {
            if (($index = array_Pesquisar($url, $this->_link_list)) === false) {
    ...
    private function _convert_pre(&$text)
    {
        ...
        while (preg_match('/<pre[^>]*>(.*)<\/pre>/ismU', $text, $matches)) {
            ...
            // Run our defined tags Pesquisar-and-replace with callback
            $this->pre_content = preg_replace_callback($this->callback_Pesquisar,
                array($this, '_preg_callback'), $this->pre_content);

            // convert the content
            $this->pre_content = sprintf('<div><br>%s<br></div>',
                preg_replace($this->pre_Pesquisar, $this->pre_replace, $this->pre_content));
